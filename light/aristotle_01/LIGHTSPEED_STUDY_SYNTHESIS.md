...
>
> Audited in `SUBSTRATE_LIGHTSPEED_REPORT.md` §8, with proofs in
> `RequestProject/SubstrateConstants.lean`.  P2 (`0.02938 %`), P7 (`0.01962 %`)
> and P8 (`0.0000374 %`, better than quoted) are confirmed.  Two corrections:
>
> * **P4's residual is `0.00919 %`, not `0.007 %`.**  "Priority 1: the 0.007 %
>   mass residual" should read `0.0092 %`; in units of `α² = 5.325×10⁻⁵` the
>   residual is `1.726 α²`, not `1.35 α²`.
> * **P6 is a tautology.**  `⌊MONAD⌋ = 13`, so `MONAD = 13 + WOBBLE` and
>   `MONAD/13 = 1 + L` is the definition of `L` rewritten
>   (`monad_div_thirteen`).  The resulting `v/c = 0.338878` is a definition of a
>   substrate velocity, not a prediction.
> * **P5 holds on the Golay layer only.**  Among Leech minimal vectors the
>   substrate's own tax audit ranks class A (`4.5294`) below the octad class B
>   (`6.1174`).  The correct statement is that the octads uniquely minimise the
>   symmetry tax among *nonzero Golay codewords*, at `8Y + 1 = 3.1174`.
>
> The document's own conclusion — that deriving `c` is structurally closed by
> Buckingham's Π theorem, and that the substrate's role is to supply
> dimensionless ratios against SI-defined anchors — is confirmed and is now
> proved in the specific form that applies (`speed_not_from_action_and_energy`).
