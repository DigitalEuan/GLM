/-
# The substrate "speed of light": what the calibration chain does and does not say

This file is **retrieved material**: it restates, against this repository's own
constants, the exact-rational calibration chain that an earlier round of the
GLM work (`source_material/GLM-main.zip`, `light/aristotle_01` and
`light/EM_calibration_1`) built to test the claim that the speed of light is an
*output* of the substrate rather than an input to it.

The chain, as the source note writes it, is
...
Since the 2019 SI redefinition `c`, `h` and `N_A` are *exact rational numbers*,
so the whole chain is exact rational arithmetic — no floats, by directive D7 —
and is developed here over `ℚ`.

What the file establishes:

* `cellLength_bounds` and its companions — every number the note quotes is
  correct to the digits it quotes, computed exactly.
* `cellLength_div_cellDuration` and `substrate_c_is_circular` — the claim that
  "the speed of light is not an input constant" is **false as stated**: `c` is
  used once, to convert the cell *duration* into a cell *length*, and is then
  recovered identically for *every* value of the anchor `κ` and *every* tick
  budget. The chain determines the cell length, not `c`.
* `speed_not_from_action_and_energy` — the structural reason, as a dimensional
  obstruction: an action and an energy generate only the dimensions
  `Mᵃ⁺ᵇ L²ᵃ⁺²ᵇ T⁻ᵃ⁻²ᵇ`, which never contains `L T⁻¹`. A calibration that
  supplies an energy scale can fix a time but never a velocity
  (`time_from_action_and_energy`), and an independent *length* is exactly what
  the substrate does not supply (`speed_from_length_and_time`).
* What survives is dimensionless and falsifiable: the propagation-speed law
  `n(T) = (24+T)/(24+T₀)` (`refIndex`), which contains no empirical input at
  all, forces the reference TAX `T₀` to be the *minimum* admissible TAX if the
  model is to be causal (`signalSpeed_le_c_iff`), and caps the refractive index
  at `16/9` if the TAX cannot exceed 24 (`refIndex_le_of_tax_le`) — a cap that
  diamond's `n = 2.417` already exceeds.
* `tax_indicator` ties the tick budget to this repository's own cost function:
  the tax of a `0/1` carrier of Hamming weight `w` is `w·Y + w/8`, which is
  `GLM.tax` of `Constants.lean` and no new definition.

`GolayWeightEnum.lean` supplies the remaining half of the story — which weight
`w` actually minimises that tax over the Golay code, and hence where the `3` of
"24 bits + 3 TAX" comes from.
-/
import Mathlib
import RequestProject.GLM.Constants

namespace GLM.Lightspeed
...
whatsoever on `c`. Had the substrate's true speed of light been any other value
`c'`, running the identical chain with `c'` in place of `c` would return `c'`.
Hence the chain cannot be used as evidence for the value of `c`. -/
...
the elementary form of the Buckingham Π obstruction: no amount of dimensionless
substrate structure can produce a dimensionful constant. -/
...
/-- An action and an energy do not determine a *mass* either. -/
...
/-! ## 7. The tick budget and this repository's cost function

The `T` of `ticksPerCell` is the substrate's symmetry tax, which
`Constants.lean` already defines: `TAX(v) = HW(v)·Y + ‖v‖²/8`. On a `0/1`
carrier — which is what a Golay codeword is — the two terms collapse into one
weight, and that is the only bridge the light chain needs. -/

/-- The tax of a weight-`w` `0/1` carrier: `w·Y + w/8`. -/
noncomputable def codewordTax (w : ℕ) : ℝ := (w : ℝ) * GLM.Y + (w : ℝ) / 8
...
theorem octadTax_eq : codewordTax 8 = 8 * GLM.Y + 1 := by
  rw [codewordTax]; push_cast; ring

theorem Y_bounds : 0.264675 < GLM.Y ∧ GLM.Y < 0.264676 := by
  have h1 : (3.141592 : ℝ) < Real.pi := Real.pi_gt_d6
  have h2 : Real.pi < 3.141593 := Real.pi_lt_d6
  have hpi : (0:ℝ) < Real.pi := Real.pi_pos
  have hY : GLM.Y = Real.pi / (Real.pi ^ 2 + 2) := by
    rw [GLM.Y]; field_simp
...
/-- **Where the `3` of "24 bits + 3 TAX" comes from.** `⌊Tax(octad)⌋ = 3`: the
overhead the note rounds to `3` is the integer part of the tax of a weight-8
carrier. `GolayWeightEnum.lean` proves that weight 8 is the minimum available
over the nonzero Golay codewords, so this is the minimum nonzero tax on the
code layer. -/
...
/-! ### The octad is not the global minimum-tax state

At the *Leech* layer all minimal vectors have `‖v‖² = 32`, so their tax is
`w·Y + 4`, and the class of shape `(∓4², 0²²)` — Hamming weight 2 — is strictly
cheaper than the octad class `(∓2⁸, 0¹⁶)`. So "photon = minimum-tax octad" is
true on the Golay layer and false among Leech minimal vectors. -/

/-- Tax of a Leech minimal vector of Hamming weight `w`: all minimal vectors
have `‖v‖² = 32`, so `Tax = w·Y + 4`. -/
noncomputable def minimalVectorTax (w : ℕ) : ℝ := (w : ℝ) * GLM.Y + 4

theorem classA_tax_lt_octad_tax : minimalVectorTax 2 < minimalVectorTax 8 := by
  have := GLM.Y_pos; rw [minimalVectorTax, minimalVectorTax]; push_cast; nlinarith
