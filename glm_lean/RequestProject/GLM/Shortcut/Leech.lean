import RequestProject.GLM.Shortcut.GolayWeights
...
* `GLM.LatticeShortcut.leech_min_norm` — every nonzero lattice vector has
  `‖x‖² ≥ 32` (the kissing-sphere radius);
* `GLM.LatticeShortcut.golay_step_isLeech` — the doubled difference of **any** two
  Golay codewords is a Leech vector, of norm `4 · d(c₁,c₂)`;
* `GLM.LatticeShortcut.golay_step_minimal_iff` — such a step is a *minimal*
