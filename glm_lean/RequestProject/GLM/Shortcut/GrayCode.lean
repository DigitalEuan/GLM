...
* `GLM.LatticeShortcut.normSq_eq_d2` : `‖Δv‖²` really is the Hamming distance;
* `GLM.LatticeShortcut.d2_eq_pop_gray_xor` : the **O(1) shortcut formula**
  `d²(a,b) = pop (gray (a XOR b))` — the jump norm never requires walking the
  interval between `a` and `b`;
* `GLM.LatticeShortcut.d2_succ` : adjacent integers are always at `d² = 1`
...
* `GLM.LatticeShortcut.d2_mod_two` / `d2_even_iff` : the exact parity law
  ...
  proved in `RequestProject.GLM.Shortcut.Substrate` (`legacy_even_quantisation`).
-/
