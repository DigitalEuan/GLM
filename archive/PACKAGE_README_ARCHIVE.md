...
*Moved into `archive/` from `overlay/`, where it was `README_ARCHIVE.md`. Only
the relative link prefixes were adjusted for the new location; no sentence and
no figure was touched.*

The archival half of [`README.md`](../overlay/README.md): the change log, and the
...
is now, see [`README.md`](../overlay/README.md) and [`FIGURES.md`](../overlay/FIGURES.md), which is
...
| 5.23 | 2026-09-07 | Generated instead of stored, and checked against what it replaces. The attached zero-storage concept — regenerate the Leech lattice from a sieve, hold a real number as a process, a register as a loop — is measured by `reasoning/generative.py` and `report generated`, and formalised in `RequestProject/GLM/ZeroStorage.lean`. **The principle holds and pays**: the audited tables cost **9,449,445 bytes** stored against **24,648** for their generators, about **383 : 1**, with every regenerated object compared against the stored one; and **7,296,569 of the 7,316,334 bytes** this package keeps on disk are already caches with a digest, against **19,765** bytes of primary data. **The proposed sieve does not**: it is sound (`v3Sieve_sound`) and keeps **1,152 of the 196,560** minimal vectors — every `(±4²,0²²)`, 48 of the odd ones, none of the 97,152 octad vectors — because "all coordinates agree mod 4" is not the Golay condition of Construction C; `v3Sieve_iff` says exactly what it generates, and `octadVec_not_v3Sieve` is a rejected minimal vector. The one-line repair agrees with the package's membership test on all **196,656** vectors tested. The snap built on the sieve returned a non-lattice point on **4 of 4** targets in general position and **3 of 4** beside a minimal vector (`fallbackVec_not_isLeech`), where the exact coset decoder added here is always inside Λ and within the covering radius. Of the closed-form constants **0 of 3** stated accuracies hold — ln 2 gives 9 bits where 256 are claimed, γ gives 3 — and the Babylonian denominator doubles every step, so the module's own default of 64 iterations cannot be run: a process is a number only when its error is a function of the work. New: one Lean file (**98 Lean files**, no `sorry`), one test file (**75 test files**), one reasoning module (**60 modules**), the `report generated` subject (**52 report subjects**) and the evaluation case that drives it through the CLI (**135 CLI cases**, 135 / 135). Nothing in the module reads a clock: the audit is emitted through the runtime, whose traces must be byte-identical between runs, so the storage rows carry bytes and verdicts and no timing. Write-up: `studies/ZERO_STORAGE_STUDY.md`. |
| 5.24 | 2026-09-08 | The last stored table removed, and the cost of generating measured. `glm_zero_storage_substrate_v5.py` (standalone, standard library only, `int`/`Fraction`, no float, no RNG) replaces v4's 4096-word Golay lookup by the **12-bit syndrome**: the code is self-dual, so its **12 generator rows** are a parity-check matrix and membership is twelve AND-popcount-parity operations against **36 bytes**, with the syndrome returned instead of a bare `False`. `GLM.ZeroStorageV5.syndromeZero_iff_isGolay` proves the equivalence and `syndromeSieve_iff_isLeech` carries it to the whole Leech membership test; the two routes were compared, before the lookup was removed, on the **196,560**-vector shell (**0 disagreements**), on 1,536 non-codewords, on the entire **16,777,216**-word space and on the Lean development's own differently generated rows. Every answer now carries an exact integer **cost ledger** (13 primitives, never wall-clock, reproducible byte for byte), so the storage audit is stored bytes against generator bytes **plus the tax**: 12 parity checks for a membership decision, about one XOR per codeword, **50,705** primitives for a pruned nearest-point decode against **217,272** unpruned — the pruned coset search stores nothing and returns the same point at the same distance. **NRCI** is defined once — `1 − √(Σr²/Σx²)`, an exact dyadic enclosure of width `2⁻ᵏ` plus the rational squared form — and reported on three named streams, with the proved `1/N` bound printed beside the Δ-Σ figure. The register retargets **continuously** (`ds_track_bound`, `ds_track_moving_target`), where the v4 zeroing register held one tick of evidence on the same trajectory; second- and third-order noise shaping is **measured, not assumed** — the guaranteed filtered bound improves from `1.9 × 10⁻³` to `1.3 × 10⁻⁵` at N = 1024, and order 3 at these coefficients is unstable; and the coset decoder's repair step is proved optimal (`coset_cost_ge`, `coset_repair_attained`). New: one Lean file (**99 Lean files**, 28,884 lines, 2,881 parsed declarations, no `sorry`). Write-up: `studies/ZERO_STORAGE_V5_STUDY.md`. |
| 5.25 | 2026-09-08 | The decoder proved optimal on the whole lattice, and the corpus figures re-measured. `GLM.ZeroStorageV5` closes the gap the previous round left open: `coset_min_cost` bounds every point of a Construction-C coset below by the per-coordinate nearest point plus, when the mod-8 sum forces an odd repair, the cheapest single ±4 penalty; `coset_min_attained` exhibits a point of the coset that achieves it; `leech_in_coset` puts every Leech point in the coset named by its own parity and mod-4 mask, so the **8,192** cosets exhaust `Λ₂₄`; and `lattice_dist_ge` carries a per-coset bound to the whole lattice — so the decode is the *global* nearest point, not merely the nearest in its coset. Rational-target versions of the per-coordinate lemmas come with them, and `allTwos_inCoset` witnesses a coset that really occurs, so none of it is vacuous. The Lean tree grew with the proofs, so every derived figure was recomputed rather than patched: **99 Lean files, 29,122 lines, 2,893 parsed declarations, no `sorry`**; the address book and the lexical book regenerated against the new tree digest; `studies/LEAN_ADDRESS_STUDY.md` re-measured (read back **2,893 / 2,893** with 0 coordinate errors, 2,547 distinct addresses, nearest-by-address shares a file **587 / 2,893** against 35 for the digest control and 26 for the reshuffle, chance ≈ 1.32 %) and `studies/ADDRESS_RETRIEVAL_STUDY.md` re-measured over **207** queries (hit@5 **51.7 %** against **6.7 %** chance, the text control still ahead at **85.0 %**, completeness bound **147,492** pairs, **0** violations). Full suite green: **3,181 tests, 15,253 subtests, 26 skipped, zero failures**, and the release sign-off signs every one of the 75 test files and every instrument. |
| 5.26 | 2026-09-09 | The corpus itself made data, and the counts it moved reconciled. `glm_universal/corpus/` (6 modules, `tests/test_corpus.py`, 43 tests) holds the project's documents the way the substrate holds data: an inventory that classifies every document by rule — archive membership decided by the path, never by judgement — the generated `DIGEST.md` and in-document `<!-- generated: … -->` blocks, a Leech address for all 688 sections of the corpus with a shortlist that is complete up to a stated radius, the measurement cache the address study's six results tables are emitted from, and the checks that fail when any of it drifts. `ENTRY.md` states the reading order and a tested coverage claim; every current-state document opens with a tier-0 block, so a coarse read of the whole project costs about 2,400 words against roughly 189,000 for the full current state. `RequestProject/GLM/Corpus.lean` is the part of it that is a theorem: `coarse_read_sound`, `card_state_add_card_archive`, `value_eq_of_fresh` and `absent_of_shortlist_empty`. The round then re-earned every figure the growth had moved: the development is **100 Lean files, 29,370 lines, 2,913 declarations, no `sorry`**, the suite is **76 test files**, and the address book, the lexical book and the Lean measurement cache were all re-taken against the new tree digest — read back 2,913 / 2,913 with 0 coordinate errors, 2,566 distinct addresses, nearest-by-address sharing a file 591 / 2,913 against 35 for the digest control and 34 for the seeded reshuffle. `D10 — a document is data` is the standing rule this leaves behind, with `python3 -m glm_universal.corpus --check` as its instrument. |
| 5.27 | 2026-09-09 | **The wobble landscape, pre-registered and measured.** A study written and committed *before* the measurement module existed: one statistic, two nulls, a two-sided tail, a multiplicity correction and a gate, fixed in `studies/WOBBLE_LANDSCAPE_STUDY.md` first. `reasoning/wobble_landscape.py` (61st reasoning module, `report landscape`, `tests/test_wobble_landscape.py`, 35 tests) implements the closed-form gap spectrum of a Sturmian word from the continued fraction — `long(K) = ceil((K+1) frac(1/t)) - 1`, checked against a 20,000-bit run — and scores the fine-structure constant against two magnitude-matched nulls enumerated exhaustively in exact arithmetic. **B = 1.79 bits** against the primary stride null (tail `77/1066`, 3.79 bits raw, corrected for 4 statistics tried), which is *weak* by the pre-registered gate, so the landscape enumeration was not run; the secondary `k`-sweep null scores below zero and the disagreement is reported as a fact about that null's measure. Two older readings were corrected: the catalogue's wobble entropy 0.062 for alpha is exactly `H2(alpha)` and carries only the magnitude, and `d_min <= 3` against the Golay code is the *majority* case — `4096 × 2325 = 9,523,200` of `2^24`, `2325/4096`, about 0.82 bits — while against a magnitude-matched null alpha's `d_min = 0` is worth **0 bits**, since all 1,066 members reproduce it. `RequestProject/GLM/WobbleLandscape.lean` carries the three theorems: the two-distance gap bound, the sphere count instantiated on the substrate's own Golay code, and antitonicity of the bit score. `glm_universal.__version__` 1.15.0 → **1.16.0**. |
| 5.28 | 2026-09-10 | **The caches the previous round left stale, re-taken, and the suite re-counted.** No new claim: a reconciliation round that ran every instrument rather than trusting the last report. The corpus address book, `DIGEST.md` and the generated blocks of `studies/CORPUS_ADDRESS_STUDY.md` were stale against the documents Phase 31 wrote — the corpus is now **708** addressable sections over 64 documents, read back 708 / 708 with **0** coordinate errors of 16,992, and `python3 -m glm_universal.corpus --check` reports `current`. The sign-off ledger's suite totals still described a 76-file suite, so five documents quoted a sentence no run had produced; a full release run re-measured them — **3,257 tests across 76 of the 77 test files, 12,703 subtests, outside the document check**, every one of the 77 test files and all seven instruments passing — and `overlay/FIGURES.md` was regenerated (it had also frozen the package version at 1.15.0 against the code's 1.16.0). `STATUS.md`'s hand-typed corpus figures were re-derived with them: 688 sections → **708**, and the tier-0 read **2,548** words against roughly **194,000** for the full current state. `lake build` completes over the Lean development, **101 files, no `sorry`**, and the two copies of the tree agree. |
| 5.29 | 2026-09-10 | **The four things the untouched list had been carrying, and the figures the growth moved.** Four items that had stood as stated limitations rather than mechanisms are closed in the shape D5 requires — a report subject, a test file, a study and a Lean file each. **The cross-register analogy**: `reasoning/conjugate.py` holds 7 energy domains — an intensive effort, the extensive extent it acts through and the transfer they make, 21 names over `effort_of`, `extent_of` and `conjugate_of` — every row checked against the physics register in exact integer arithmetic, no name in two columns, so `heat : temperature :: force : work` is derived in either direction; 11 transport questions come out 8 answered and 3 refused, each refusal naming which of `determinate`, `role_typed`, `functional`, `grounded` failed (`Conjugate.lean`). **Sparse chemistry**: every one of the 395 unmeasured element cells is decided — a rule is admitted only for halving the field's own mean out of sample on at least 20 elements, 9 of the 14 fields take one, 185 cells are filled by estimate (coverage **1,257 → 1,442 of 1,652**) and the 210 left are 100 inputs absent, 97 no admitted rule, 13 not derivable; nothing is written back (`Completion.lean`). **The vague `related_to` triples**: a router of four routes tried in order, only the last asking a person — **34 of 66** decided without one, and **1 of 4** proposer rules admitted, the three refused kept with the disagreement that refused them (`Vagueness.lean`). **Open vocabulary**: a name is admissible exactly when a stated route gives it coordinates computed from a register the machine already checks; three routes admit and one refuses, **20 of 27** probes admitted, the refusal conditional and naming its condition, and the held vocabulary of 1,093 names unchanged (`Admission.lean`). The growth moved every derived figure, so each was re-measured: **105 Lean files, 30,853 lines, 3,049 parsed declarations, no `sorry`**, **81 test files**, the address book and the retrieval cache re-taken against the new tree digest — read back **3,049 / 3,049** with 0 coordinate errors, 2,688 distinct addresses, nearest-by-address sharing a file **609 / 3,049** against 37 for the digest control and 26 for the reshuffle — and retrieval over **204** queries at hit@5 **48.5 %** against **6.4 %** chance, the text control still ahead at **80.9 %**, the completeness bound holding on **155,448** pairs with **0** violations. A full release run re-earned the suite sentence — **3,379 tests across 80 of the 81 test files, 12,792 subtests, outside the document check** — with every test file and all seven instruments passing, the evaluation **141 / 141** with the same 16 boundary refusals, and `glm_universal.corpus --check` reporting `current` over a corpus that is now **754** addressable sections. Deliberately not done: no estimate written back into the element register, the 32 referred triples not decided by hand, the refused proposer rules not tuned until they passed, and the Niemeier deep holes not attempted — they stand alone as Phase 33. |
| 5.30 | 2026-09-11 | **The Niemeier deep holes, classified from trajectories — and stopped by their own sanity check.** A pre-registered study (`reasoning/deep_hole_classifier.py`, `DeepHoleClassifier.lean`, `studies/DEEP_HOLE_STUDY.md`, `report hole classifier`, `tests/test_deep_hole_classifier.py`) asks whether the distribution of arrivals of a declared 240-start walk ensemble can name a deep hole's Coxeter–Dynkin type: arrival shares as the statistic, a nearest-reference rule with a stated refusal, and four controls of which the plain vertex count is the competitor that matters. It names **15 of 44** queries against **11** for the vertex count, **12** for the uniform-profile ablation, **4** for the digest control and **3** for the reshuffle — beating every control — and the round stops anyway, because the pre-registered sanity query fails: change only the ensemble seed and **3 of 10** holes keep their label. The Lean half proves what the method claims rather than what it scored: invariance of the statistic under relabelling, totality with a refusal that is a value, and certified absence under a separation hypothesis. |
| 5.31 | 2026-09-11 | **The deep-hole ladder: the same question read one layer up.** The second half of the same question (`reasoning/deep_hole_escalation.py`, `DeepHoleEscalation.lean`, `studies/DEEP_HOLE_ESCALATION_STUDY.md`, `report hole ladder`, `tests/test_deep_hole_escalation.py`) asks whether the failure was the geometry or the layer it was read at, and escalates along a declared ladder of layer × budget cells — shares, shares widened by the strays the first round discarded, an exact rational 1-Wasserstein reading of emission distances, and the two joined, each at 240, 480, 960 and 1920 nested starts. The sanity count climbs `3 → 5 → 8` on budget alone and `3 → 4 → 6` on width alone, reaches **9 of 10** at the best pre-registered cell and **10 of 10** at the joint reading with 1920 starts, where the full query set is **40 of 44** against 12 for the vertex count and the mean rank of a query's own reference is **1.09**. Two things are recorded against it: the rational reading alone conflates `A_1^24` with `A_2^12`, and the separation criterion `ρ = 2W/B < 1` still fails everywhere (best **2.59**), so the classifier names holes and still cannot certify an absence. |
| 5.32 | 2026-09-11 | **The measurements the deep-hole rounds moved, re-taken.** No new claim: a reconciliation, run rather than reported. The lexical address book and the two address studies' measurement cache were stale against the new tree digest, so every generated table in `LEAN_ADDRESS_STUDY.md` and `ADDRESS_RETRIEVAL_STUDY.md` was printing its staleness notice instead of a figure; both are recomputed and the hand-written passages around them were brought to the new measurements — **3,100 / 3,100** read back with **0** coordinate errors, 2,736 distinct addresses, nearest-by-address sharing a file **609 / 3,100** against 36 for the digest control and 18 for the reshuffle at ≈ **1.21 %** chance, and retrieval over **207** queries at hit@5 **39.1 %** against **6.3 %** chance with the text control ahead at **85.0 %** and the completeness bound holding on **154,950** pairs with **0** violations. The worked examples of both studies were re-run rather than re-picked. A release run re-earned the suite sentence — **3,436 tests across 82 of the 83 test files, 12,836 subtests, outside the document check** — and the evaluation row is **143 / 143** over 143 CLI cases, the development **107 Lean files, 31,483 lines, 3,100 declarations, 0 `sorry`**, the package **121 modules**. |
| 5.33 | 2026-09-12 | **The proof-carrying generative substrate: "generate, don't store" with evidence attached.** The two supplied PCGS scripts could not be run here — both import a package from an absolute path outside the repository, one of them `chdir`s into a tree that does not exist — so the mathematics in them was rewritten against this project's own substrate as `reasoning/pcgs.py`, with `RequestProject/GLM/PCGS.lean`, `studies/PCGS_STUDY.md` and `tests/test_pcgs.py` (**45 tests, 1,048 subtests**). Six systems are admitted against the concept's six criteria, and each states the *kind* of evidence it has: three proved — the cost algebra (`Cost.plus_comm`, `plus_assoc`, `algebraicTotal_plus`, `algebraicTotal_scale`) and the information axis (`bitsFor_le_iff`), Reed–Muller `RM(1,m)` (`rmWeight_of_ne_zero` by a coordinate-flip involution, hence `rm_min_distance` = `2^(m-1)` with no codeword table), and the physical layer (`bitsErased_reversible`, `landauerEnergy_mono`, `landauerEnergy_le_of_le_log_two`); one a checked transcription — `ntt_intt` proves the transform pair generated from `(p, g)` inverts, and the radix-2 algorithm is checked against that definition on every input rather than claimed; and two tested — a stencil operator and a transducer. Caching is priced rather than deprecated: `breakevenQueries_spec` proves `⌈store/(gen−look)⌉` is exactly the threshold, which puts the Golay codeword table at **8,937** membership queries. Three claims of the source scripts do not survive checking and are corrected: "`RM(1,m)` is self-dual iff `m` is odd" (only `m = 3` is), an efficiency of `0` returned for the undefined case, and a float fallback in the information bound above 65,536 symbols. One improvement: the script's `ln 2` lower bound is the alternating harmonic sum, still wrong in the third decimal at a hundred terms; `ln2Fast_le_log_two` proves that **every** partial sum of `Σ 1/(k 2^k)` is a lower bound, and at 64 terms the residual is below `10^-21`. |
...
## The `glm_universal` package README's own changelog

*Moved here from `overlay/glm_universal/README.md`, unchanged, when that
README was rebuilt: a 740-line version history inside a current-state
document is a record of rounds, and the archive rule says where a record
of a round lives. The counts in it are the counts of the round that wrote
them and are meant to be out of date.*
...
- Two new test files, `test_retrieval.py` (42) and `test_controller.py` (25);
  ...
  controller` are the newest of the **53 report subjects** and the newest two
  of the **136 CLI cases**, and the end-to-end set was **134 / 134** at that
  release, with the same 16 boundary refusals.
- The address book is regenerated whenever the tree moves; over the current
  101-file tree it reads back **2939/2939** exactly, 0 coordinate errors,
  2588 distinct addresses, and nearest-by-address shares a file 600 / 2,939
  against 37 for the digest control and 19 for the seeded reshuffle. The
  tables that state this are generated blocks in
  `studies/LEAN_ADDRESS_STUDY.md`, so they report staleness rather than an
  out-of-date number.
