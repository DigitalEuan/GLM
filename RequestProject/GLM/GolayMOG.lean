/-
# The Gray/Golay/MOG data object's discrete layer

Retrieved from the archive's spatial-arithmetic experiments
(`data_object/encoding_definition_attempt_03-08.26/RequestProject/GolayMOG.lean`),
where it was the one Lean file of the study.  The chemistry correlations of that
study are the archive's own figures and are not re-run here; what is retrieved
is the part that is true of the encoding whatever the chemistry says.

* `decodeIdentity_identityAddress`, `identityAddress_injective` — the 12-bit
  Gray identity address of an element (atomic number 1–118) decodes back to its
  atomic number, so the 118 addresses are collision-free.
* `consecutive_identity_oneBitApart` — consecutive atomic numbers get addresses
  exactly one bit apart: the Gray layer makes the periodic table's order local.
* `mogCoordinate_bijective`, `octadZoneCoordinate_bijective` — the fixed 4×6
  MOG cell assignment loses and duplicates no coordinate, and its three
  eight-coordinate column-pair regions partition the 24 coordinates.
* `leechAddress_sqNorm`, `leechAddress_injective`,
  `leechMinimalClass_counts`, `leechMinimalClass_representative_sqNorms` — the
  24 stored Leech addresses are minimal vectors (integer-scale norm 32, norm 4
  after the `1/√8` rescaling) and pairwise distinct, and the three classical
  shape families count `1104 + 97152 + 98304 = 196560`.
* `symmetricInteraction_comm`, `symmetricInteraction_self`,
  `symmetricInteraction_recover`, `interactionABC_symmetric`,
  `interactionB_self` — the declared interaction operators are symmetric, and
  the XOR contrast recovers either input from the other.

Five results are **negative**, and they are the reason the file is kept:

* `binaryTax_mono` — on a binary vector TAX is a monotone function of Hamming
  weight alone, so it carries no ordering information beyond the weight;
* `binaryNRCI_above_half` — below weight 16 an NRCI above one half is automatic
  whenever `Y < 3/16`, so that threshold selects nothing;
* `element_relativeCoherent_seventy_percent` — under the observed score bounds
  the "retain 70 % of a peer's score" rule is passed by every element, so it
  is non-selective;
* `yTwin_injective` — the "virtual Y twin" appends a fixed multiple of each
  coordinate, which is injective but adds no information;
* `projection24to3Q_not_injective` — and the published 24-to-3 Walsh
  visualisation is necessarily lossy: two coordinate basis vectors share an
  image.
-/
import Mathlib

namespace GLM.GolayMOG
