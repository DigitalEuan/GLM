import RequestProject.GLM.Shortcut.Golay
...
* `GLM.LatticeShortcut.golay_weight_distribution` — weight enumerator
  `1 + 759 x⁸ + 2576 x¹² + 759 x¹⁶ + x²⁴`;
* `GLM.LatticeShortcut.golay_weight_div_four` — the code is doubly even;
* `GLM.LatticeShortcut.golay_min_dist` — minimum distance `8`;
* `GLM.LatticeShortcut.isGolay_iff` — codeword recognition is an `O(1)` test.
-/
