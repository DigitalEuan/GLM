import RequestProject.GLM.Shortcut.Decoder
...
coordinates `8k … 8k+7`.  That map is `GLM.LatticeShortcut.grayBytes`, and it is
...
`RequestProject.GLM.Shortcut.Decoder`, not the weight-`≤ 3` corrector of the substrate.

Main results:

* `GLM.LatticeShortcut.corrected_step_isLeech` — every corrected transition,
  doubled, is a genuine Leech lattice vector;
* `GLM.LatticeShortcut.corrected_quantized` — its squared norm is
  `4 · d² with d² ∈ {0,8,12,16,24}`; the true quantisation is by `4`
  (equivalently, the doubled norm is quantised by `32`), not by `2`;
* `GLM.LatticeShortcut.corrected_octad_iff_minimal` — the transition is a
...
`RequestProject.GLM.Shortcut.Substrate.legacy_even_quantisation`. -/
