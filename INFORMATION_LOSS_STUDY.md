...
* **`glm_universal_v0.6.0/glm_universal/reasoning/information_loss.py`** — the
...
each handoff: the vacuum; `1/2`, `1` and `2` on coordinate 0; and `1` on
coordinate 10 (outside the seven SI exponents the integer layer reads).
...
|---|---|---|---|---|---|
| substrate | 24 | 2²⁴ | 3 / 5 | 2 | no |
| integer | 7 | unbounded | 3 / 5 | 2 | no |
| rational | 10 | unbounded | 5 / 5 | 0 | **yes** |
...
only at the rational layer, and the witnesses are produced explicitly and
re-checked against the definition rather than trusted.

### 3.1 An audit finding: the ladder has a hole

`refinement_violations` looks for the opposite of a boundary — pairs the
*lower* layer splits and the *higher* one conflates. In a true refinement chain
there are none. In the shipped stack there are two:

```
substrate → integer :  (vacuum, unit-at-coord-10)  and  (half, unit-at-coord-10)
```

The substrate perceives a 24-bit parity view, so it separates a unit on
coordinate 10 from the vacuum. The integer layer perceives only the seven SI
exponents, so it does not. Escalating from the substrate to the integer layer
therefore **destroys a distinction the layer below already had**.

By `Visible.mono` this is exactly the situation in which the cumulative
guarantee fails: because `Refines integerLayer substrateLayer` does not hold on
the real 24-coordinate carriers, a proposition visible at the substrate need
not remain visible at the integer layer. Something true below really does
become unstatable above — not by design, but by a gap between the two
perceive maps.

This is reported by the system about itself (`refinement_chain_intact = False`)
and is pinned by a test. It is the one place where the informal thesis, taken
in its strongest form — *the former becomes untrue* — is literally realised in
the implementation, and it is realised as a defect rather than as a feature.
Closing it means either widening the integer layer's view beyond the seven SI
exponents, or narrowing the substrate's parity view to the same coordinates.
...
6. **The method finds real defects.** Applying the same definitions to the
   shipped code, rather than to an idealisation of it, located a genuine
   refinement hole between the substrate and integer layers (§3.1).
...
| `glm_universal/tests/test_information_loss.py` | 42 tests pinning every number quoted in §3 |
