...
**Question.** What can the machine actually do, and where exactly does it stop?

**Verdict.** This document does not describe the machine. It reports what happened when the machine was run.

**Deciding figure.** 33 probes: 20 hold, 13 break, 0 errored, 0 surprises.
...
cases to 11 since it was fixed. And there are **no unexpected refusals at all**: the
...
* Every `report` subject — all 63 of them — answers from a fresh interpreter,
  and each recomputes its figures rather than quoting them. 65 / 65 in the CLI
  evaluation.
* Analogy: 11 / 11 in the CLI evaluation and 35 / 35 across the three analogy
  suites, with two of the eleven CLI cases being refusals the machine now knows
  how to make.
