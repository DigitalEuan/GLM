...
| 8 | `planner-utility` | no-discard | the whole 147-case evaluation set under the declared fallback rule | nothing.  Every question in the evaluation set that the runtime refuses is a question that ought to be refused, so the set contains no headroom for the planner; the five gains on the declared task set are all off-set |
<!-- end generated -->
