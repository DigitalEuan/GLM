...
1. **Yes, on all three sets.** Gated on the text layer's own confidence, the
   ...
   the ladder. The standing has moved with the corpus and is recorded as it
...
**The stack beats the faculty that beat the geometry on every set.** At k = 5
...
`GLM.Gen3.m24_orbit_stabiliser`), and empty again after Phase 61 added
`Distinction.lean`, `SeedRoles.lean` and `GolayMOG.lean` — which is why the
column stays in the table rather than in a footnote, and why the verdict is
...
each set, with precision@5 up by a fraction of a point. Recorded as it
