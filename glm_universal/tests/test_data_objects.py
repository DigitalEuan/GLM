...
class TestPhysics(unittest.TestCase):
    ...
    def test_register_holds_720_concepts(self):
        self.assertEqual(len(self.register), 726)
        self.assertEqual(len({q.name for q in self.register}), 726)
    ...
    def test_ext10_resolves_more_than_si7(self):
        ...
        self.assertEqual(report["concepts"], 726)
        ...
        for domain in ("physics", "chemistry", "mathematics", "lexicon"):
            for obj in catalogue[domain]:
                with self.subTest(domain=domain, name=obj.name):
                    self.assertTrue(obj.round_trip_ok())
                total += 1
        self.assertGreaterEqual(total, 726 + 118 + 20 + 10)
