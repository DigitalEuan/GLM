...
| 2 | `data_objects/` — physics (**726 quantities**), chemistry (118 elements + 52 diatomics), **51 molecules**, mathematics (22), semantic lexicon (149), spatial (28), **45 comparison classes** | 373 | ✓ complete |
| 3 | `reasoning/` — **87 modules**: admission, analogy, analogy_models, anonymous, blockers, blueprint, catalog, coherence, column_extremum, combiner, companion, conjugate, containers, controller, coordinate_order, cumulativity, deep_dive, deep_hole_classifier, deep_hole_escalation, deep_hole_failures, deep_holes, denotation_view, dimension_layers, directives, drift, economics, element_completion, element_coverage, engine, escalation, exact_real, exactness, facets, field_surface, fwht, fwht_decode, generative, harmony, higher_lattices, information_loss, ladder_escalation, lean_address, lean_book, llvq, llvq_table, mantissa, measure_view, metric, monster_stack, moonshine, multires, name_coordinate, niemeier, noise_lab, norm_escalation, now_float_control, now_receipt, operation_escalation, pcgs, periodic_table, pipeline, probe_oracle, product, query_escalation, real_expr, retrieval, reversible, review_sweep, salvage, salvage_second, search_loop, second_reading, shell_sigma, stability, stack, tasks, term_arithmetic, tie_break, transcendental, units, vagueness, valorani, verifier, vision_stack, voronoi_walk, wobble, wobble_landscape | 1,724 | ✓ complete |
...
| 4 | `runtime/` — parser, session, TCT engine, and the `GLM.py` CLI; **<!--figure:query-kinds-->24 query kinds<!--/figure-->**, **65 report subjects**, 8 registers | 326 | ✓ complete |
...
**Total: <!--figure:suite-->4,038 tests across 100 of the 101 test files, 15,644 subtests, outside the document check<!--/figure-->, zero failures.**
...
│   ├── parser.py              natural language query parser (23 kinds)
...
│   ├── cases.py               the 172 CLI cases, every query kind and report subject
│   ├── harness.py             run_case, run_all, evaluation_report, the scoring
│   └── __main__.py            CLI, with --only, --case, --jobs, --json, --list
├── tests/                     ← <!--figure:test-files-->101 test files<!--/figure-->
...
- **The Lean development, addressed**: `lean_address.py` gives every
  ...
  address computed from its statement, read back **3455/3455** with 0
...
### The runtime (Step 4)
**<!--figure:query-kinds-->24 query kinds<!--/figure-->** and **65 report subjects** over **8 registers** — see
